...
{foreach from=$transactions key=k item=v}
   <tr {if $v.confirmations == 0 && $v.confirmations !== null}class="unconfirmed"{/if}>
     <td><img src = "../common/img/{if $v.category === move && $v.amount > 0}testcoins.png
     ...
     <td align="center" class="intext">{$v.confirmations} 
         {if $v.confirmations > 0}
             (<span class="confirmed">
                <img src = "../common/img/confirmed.png" height="12px" style="padding-top:2px;"/>{$smarty.const.SM_CONFIRMED}
              </span>)
          {elseif $v.confirmations == 0 && $v.confirmations !== null}
             (<span class="unconfirmed">
                <img src = "../common/img/unconfirmed.png" height="12px" style="padding-top:2px;"/>{$smarty.const.SM_UNCONFIRMED}
             
              </span>)
          {/if}</td>
     <td style="font-size:12px"><a href="http://blockexplorer.com/tx/{$v.txid}" target="_blank">{$v.txid|truncate:70:"..."}</a></td>
